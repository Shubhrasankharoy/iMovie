...
export default function TvShowsPart() {
    const [castSeeMore, setCastSeeMore] = useState(false);
    const [crewSeeMore, setCrewSeeMore] = useState(false);

    const tvShows = useSelector((state) => state.personDetails?.tvCredits);

    if (!tvShows || tvShows.length === 0) {
        return <div className='text-center py-10'>No TV shows found.</div>;
    }

    const { cast, crew } = tvShows;
        ...
        <div className='mt-6 space-y-4'>
            <div>
                ...
                <ul className='pl-5 flex flex-wrap gap-4 mt-2'>
                    {(castSeeMore ? cast : tvShows.cast.slice(0, 10)).map((show, index) => (
            ...
            <div>
                ...
                <ul className='pl-5 flex flex-wrap gap-4 mt-2'>
                    {(crewSeeMore ? crew : crew.slice(0, 10)).map((show, index) => (
                        <li key={index} className={`text-white w-40 animate-fade-up animate-duration-600 rounded-lg overflow-hidden`}>
                            <MovieCard movie={show} />
                        </li>
                    ))}
