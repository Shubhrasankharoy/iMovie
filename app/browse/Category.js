...
export default function Category({ typeKey, movies }) {
  ...
  return (
    ...
    >
      <div
        // className={`absolute h-[80%] text-4xl w-16 bottom-0 left-0 bg-linear-to-r z-40 ${categoryHover && !atLeft ? "flex" : "hidden"} items-center justify-start cursor-pointer from-black/90 transition-all duration-75 text-gray-400 `}
      ...
      <div
        // className={`absolute h-[80%] text-4xl w-16 bottom-0 right-0 bg-linear-to-l z-40 ${categoryHover && !atRight ? "flex" : "hidden"} items-center justify-end cursor-pointer from-black/90 transition-all duration-75 text-gray-400 `}
